...
- **null 許容**: `NoiseTexture` 未設定時は `RenderPass` 側で `SetTexture` を呼ばない。`Texture2D.whiteTexture` のような次元の異なるフォールバックを渡すと次元不一致になるため、他パスの `Mask` パターン（`mask ? mask : whiteTexture`）は流用できない。
